...
Generate the outline now:`;
    ...
    // Extracting JSON array from responce text
    ...
    if (startIndex === -1 || endIndex === -1) {
      ...
      return res.status(500).json({
        message: "Failed to Parse Ai responce, no JSON array found.",
      });
    }

    const jsonString = cleanedText.substring(startIndex, endIndex + 1);

    // Validate If the responce is valid JSON
    try {
      const outline = JSON.parse(jsonString);
      const filteredOutline = outline.map((chapter) => ({
        title: chapter.title,
        description: chapter.description,
      }));

      return res.status(200).json({ outline: filteredOutline });
    } catch (e) {
      console.log("Failed to parse Ai responce :", jsonString);
      res.status(500).json({
        message:
          "Failed to generate valid content. the Ai response was not valid",
      });
    }
  } catch (error) {
    console.log("Error While Generating Outline:", error.message);
    return res.status(500).json({
