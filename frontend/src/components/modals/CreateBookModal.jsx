...
function CreateBookModal({ isOpen, onClose, onBookCreated }) {
  ...
  const handleGenaratedOutline = async () => {
  ...
  const handleChapterChange = (index, feild, value) => {
    const updatedChapters = [...chapters];
    updatedChapters[index][feild] = value;
    ...
    >
      {step === 1 && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 mb-6">
            <div className="flex items-center justify-center h-8 w-8 rounded-full bg-violet-100 text-violet-500 text-sm font-semibold">
              1
            </div>
            <div className="flex-1 h-0.5 bg-gray-200"></div>
            <div className="flex items-center justify-center h-8 w-8 rounded-full bg-gray-100 text-gray-400 text-sm font-semibold">
              2
            </div>
          </div>
          ...
          <InputField
            ...
            placeholder="5"
            value={numChapter}
            onChange={(e) => setNumChapter(parseInt(e.target.value))}
            min="1"
            max="10"
          />
          ...
          <div className="flex justify-end pt-4 ">
            <Button
              onClick={handleGenaratedOutline}
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="flex items-center gap-2 mb-6">
            <div className="flex items-center justify-center h-8 w-8 rounded-full bg-violet-100 text-violet-600 text-sm font-semibold">
              ✓
            </div>
          ...
          >
            {chapters.length === 0 ? (
              ...
              chapters.map((chapter, index) => (
                <div
                  key={index}
                  className="group p-4 border border-gray-200 rounded-xl hover:border-gray-300 
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <input
                      ...
                      className="flex-1 text-base font-medium text-gray-900 bg-transparent border-none 
                      focus: outline-none focus: ring-0 p-0"
                    />
                    <button
                      onClick={() => handleDeleteChapter(index)}
                      className="opactiy-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-50 transition-all"
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full pl-9 text-sm text-gray-600 bg-transparent border-none focus: outline-none
                    focus: ring-0 resize-none placeholder-gray-400"
                  />
                </div>
              ))
            )}
